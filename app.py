...
def new_session_state():
    return {
        'transcript': [], 'question_buffer': "", 'answer_buffer': "",
        'relevance_scores': [], 'filler_words': {'interviewer': 0, 'candidate': 0},
    }

# --- Threads ---
# --- Threads ---
def whisper_thread(sid, audio_queue, stop_event):
    # VAD parameters from previous step
    vad_parameters = VadOptions(min_silence_duration_ms=500, speech_pad_ms=300)
    while not stop_event.is_set():
        try:
            ...
            segments, _ = whisper_model.transcribe(
                ...
                # --- Parameter changes to reduce hallucinations ---
                # 1. Increase the no_speech_threshold to be stricter about what is considered speech.
                no_speech_threshold=0.70
            )
            full_text = " ".join(segment.text for segment in segments).strip()
            
            if full_text and sid in clients:
                client_info, room_id = clients[sid], clients[sid]['room']
                if room_id not in rooms: continue
                
                speaker_role, room_state = client_info['role'], rooms[room_id]['state']
                ...
                if speaker_role == 'interviewer': room_state['question_buffer'] += full_text + " "
                elif speaker_role == 'candidate': room_state['answer_buffer'] += full_text + " "
...
@app.route('/profiles')
def profiles_page(): return render_template('profiles.html')
...
def create_interview():
    ...
    if not profile: return "Error: A profile must be selected.", 400
    
    room_id = str(uuid.uuid4())
    interviewer_token = str(uuid.uuid4())
    rooms[room_id] = {
        'interviewer': None, 'candidate': None, 'profile': profile, 
        'state': new_session_state(), 'interviewer_token': interviewer_token
    }
    return redirect(url_for('interview_room', room_id=room_id, token=interviewer_token))

@app.route('/interview/<room_id>')
def interview_room(room_id):
    if room_id not in rooms: return "Interview session not found or has expired.", 404
    token = request.args.get('token', '')
    return render_template('index.html', room_id=room_id, token=token)

@app.route('/report/<room_id>')
def generate_report(room_id):
    if room_id not in rooms: return "Report not found or session expired.", 404
    
    room_data, room_state = rooms[room_id], rooms[room_id]['state']
    profiles = load_profiles()
    ...
    candidate_text = " ".join([e['text'] for e in room_state['transcript'] if e['speaker'] == 'candidate']).lower()
    mentioned_keywords = [kw for kw in profile_keywords if kw.lower() in candidate_text]
    if mentioned_keywords: analysis['positive'] = f"Candidate effectively discussed key skills: {', '.join(mentioned_keywords)}."
        
    avg_relevance = 0
    if room_state['relevance_scores']:
        avg_relevance = sum(room_state['relevance_scores']) / len(room_state['relevance_scores'])
        if avg_relevance < 50: analysis['improvement'] = "Candidate's answers often had low relevance to the questions asked."

    metrics = {
        "avg_relevance": round(avg_relevance), "filler_words": room_state['filler_words'],
        "keywords_mentioned": mentioned_keywords if mentioned_keywords else ["None"]
    }
    
    return render_template('report.html', profile_title=room_data['profile'], transcript=room_state['transcript'], metrics=metrics, analysis=analysis)

# --- SocketIO Events ---
@socketio.on('connect')
def handle_connect():
    global shutdown_timer, active_connections
    active_connections += 1
    if shutdown_timer:
        shutdown_timer.cancel()
        shutdown_timer = None
    sid = request.sid
    print(f'Client connected: {sid}. Total connections: {active_connections}')
@socketio.on('join')
def on_join(data):
    sid, room_id, token = request.sid, data['room'], data.get('token')
    if room_id not in rooms: return
    join_room(room_id)
    
    room = rooms[room_id]
    
    if token and token == room['interviewer_token']:
        role = 'interviewer'
    else:
        role = 'candidate'
    ...
    if role == 'candidate' and room['interviewer'] is not None:
        socketio.emit('candidate_joined', room=room['interviewer'])

    clients[sid] = {
        'room': room_id, 'role': role,
        'ffmpeg_process': subprocess.Popen(['ffmpeg', '-i', 'pipe:0', '-f', 's16le', '-ac', '1', '-ar', '16000', 'pipe:1'],
    ...
    clients[sid]['ffmpeg_thread'] = threading.Thread(target=ffmpeg_reader_thread, args=(clients[sid]['ffmpeg_process'], clients[sid]['audio_queue'],
     clients[sid]['stop_event']))
...
def on_trigger_score(data):
    sid, room_id = request.sid, data['room']
    if room_id not in rooms: return
    ...
    if room_state['question_buffer'] and room_state['answer_buffer']:
        ...
        room_state['question_buffer'], room_state['answer_buffer'] = "", ""

@socketio.on('audio_data')
def handle_audio_data(audio_chunk):
    sid = request.sid
    if sid in clients:
        try: clients[sid]['ffmpeg_process'].stdin.write(audio_chunk)
        except (BrokenPipeError, OSError): cleanup_client_resources(sid)

def shutdown_server():
    """Perform a final check before shutting down the server."""
    if active_connections == 0:
        print("No clients reconnected after delay. Shutting down server...")
        os.kill(os.getpid(), signal.SIGINT)
    else:
        print(f"Shutdown aborted. {active_connections} client(s) are currently connected.")
...
def handle_disconnect():
    ...
    active_connections -= 1
    sid = request.sid
    print(f'Client disconnected: {sid}. Total connections: {active_connections}')
    cleanup_client_resources(sid)
    
    if active_connections == 0:
        print("Last client disconnected. Server will shut down in 10 seconds if no one reconnects.")
...
def cleanup_client_resources(sid):
    if sid in clients:
        ...
        if room_id in rooms:
            room = rooms[room_id]
            if room['interviewer'] == sid: room['interviewer'] = None
            elif room['candidate'] == sid: room['candidate'] = None
            
            if room['interviewer'] is None and room['candidate'] is None:
                del rooms[room_id]
...
if __name__ == '__main__':
    socketio.run(app, host='0.0.0.0', port=5000)
>>>>>>> da83491c8ac8ee77535f52dfc8045317c6d6dd1b
